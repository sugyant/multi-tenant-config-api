...
describe('TenantConfig Route Controller Test',()=>{
	...
	test('should create New Tenant Config',async ()=>{
		...
		const resp = await request(app2).post("/v1/config/").send({tenantId:"12345678",configParam:"NZD 13000"});
	...
	test('should Update Existing Tenant Config',async ()=>{
		...
		const resp = await request(app2).put("/v1/config/12345678").send({tenantId:"12345678",configParam:"NZD 14000"});
		expect(resp.statusCode).toEqual(200);
	...
	test('should get All Configs from DB',async ()=>{
		...
		const resp = await request(app2).post("/v1/config/").send({tenantId:"GLOBAL",configParam:"NZD 1000000"});
		expect(resp.statusCode).toEqual(200);
